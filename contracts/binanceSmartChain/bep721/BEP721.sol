...
import { IBEP721 } from "./interfaces/IBEP721.sol";
import { IBEP721Receiver } from "./interfaces/IBEP721Receiver.sol";
import { IBEP721Metadata } from "./interfaces/IBEP721Metadata.sol";
...
import { BEP721Feature } from "./defaults/BEP721Feature.sol";
 ...
 * {BEP721Enumerable}.
 */
contract BEP721 is Context, ERC165, IBEP721, IBEP721Metadata, BEP721Feature {
  ...
  function balanceOf(address owner)
    ...
    returns (uint256)
  {
    require(owner != address(0), "BEP721: address zero is not a valid owner");
  ...
  function approve(address to, uint256 tokenId) public virtual override {
    ...
    require(
      ...
      "BEP721: approve caller is not token owner nor approved for all"
    );
  ...
  function _safeTransfer(
    ...
    _transfer(from, to, tokenId);
    require(
      _checkOnBEP721Received(from, to, tokenId, data),
  ...
  function _safeMint(
    ...
    _mint(to, tokenId);
    require(
      _checkOnBEP721Received(address(0), to, tokenId, data),
      "BEP721: transfer to non BEP721Receiver implementer"
    );
  }
  ...
  function _requireMinted(uint256 tokenId) internal view virtual {
    require(_exists(tokenId), "BEP721: invalid token ID");
  }
  ...
  function _checkOnBEP721Received(
    ...
    if (to.isContract()) {
      try
        IBEP721Receiver(to).onBEP721Received(_msgSender(), from, tokenId, data)
